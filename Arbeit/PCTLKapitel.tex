...
Wie in Kapitel \ref{ChapGrundlagen}, \cite{clarke1982design} und \cite{baier2008principles} gezeigt, lassen sich mithilfe von $\CTL$ viele  Eigenschaften von nicht-deterministischen System beschreiben.
...
\begin{enumerate}
	...
	Dies lässt sich mithilfe von Wahrscheinlichkeiten realisieren. 
\end{enumerate}
...
Dies ist wohldefiniert, da durch die geforderten Eigenschaften von $\mathcal{T}$, jeder Zustand eine ausgehende Kante haben muss.

Eine Markov Kette ist also ein gerichteter, gewichteter Graph, wobei die Gewichtung der Kanten angibt, wie wahrscheinlich es ist, eine bestimmte Kante auszuwählen. Zusätzlich soll die Summe aller Gewichte der ausgehenden Kanten eines Knotens immer gleich eins sein. Damit ist auch gewährleistet, dass es keine \textit{Deadlock}-Knoten gibt, welche keine weitere Transition erlauben.
...
\begin{itemize}
	\item Wählt man eine leere Folge mit Länge $0$ ergibt sich
...
Um dies zu verdeutlichen sollen einige Beispiele betrachtet werden.
...
\begin{definition}[Semantik von $\PCTL$ auf Markov-Ketten]
	...
	\begin{enumerate}
		...
		\item $s\models_\mathfrak{S} \varphi_1\land\varphi_2 \Leftrightarrow s_0\models_\mathfrak{S} \varphi_1 \text{ und } s_0\models_\mathfrak{S} \varphi_2$
...
\begin{example}[Beispiele für $\PCTL$-Formeln und deren Syntax]
	...
	Intuitiv bedeutet $s\models_\mathfrak{S} \varphi$ also, dass wenn $s\models_\mathfrak{S} X$, dann folgt, dass innerhalb von $15$ Zeiteinheiten mit einer Wahrscheinlichkeit von mindestens $90\%$, $Y$ gelten wird.
	...
	Für ein paar weitere Beispiele und Erkenntnisse soll die Markov-Kette $\mathfrak{S}$ aus Abbildung \ref{MarkovGraph1} betrachtet werden.
...
\begin{algorithm}[h]
	...
	\begin{algorithmic}
		...
		\EndIf
		\EndFor
		\State $\overline{\mathcal{P}}(t)=M^t\cdot \overline{\mathcal{P}}(0)$
	\end{algorithmic}
\end{algorithm}
